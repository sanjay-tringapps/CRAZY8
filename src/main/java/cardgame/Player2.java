...
public class Player2 implements PlayerStrategy{
	...
	public void receiveCard(Card drawnCard) {
		System.out.println("Player - 2 recieved :"+drawnCard.getRank()+" "+drawnCard.getSuit());
	...
	public Card playCard() {
		...
		if(changedSuit==null) {
			for(int i=0;i<myCards.size();i++) {
				if(myCards.get(i).getSuit().equals(topPileCard.getSuit())||myCards.get(i).getRank().equals(topPileCard.getRank())) {
					System.out.println("Player - 2 played : "+myCards.get(i).getRank()+" "+myCards.get(i).getSuit());
