		...
		while(point1<200 && point2<200 && point3<200 && point4<200) 
		{
			...
			for(i=0; i<3; i++) 
			{
				if(user1.shouldDrawCard(mainCard, decCard))
				{
					if(deckCards.size()!=0) 
					{	
						user1.receiveCard(deckCards.get(deckCards.size()-1));		
						deckCards.remove(deckCards.size()-1);			
					}
				}
				else
			...
			for(i=0; i<3; i++) 
			{
				if(user2.shouldDrawCard(mainCard, decCard))
				{
					if(deckCards.size()!=0) 
					{
						user2.receiveCard(deckCards.get(deckCards.size()-1));		
						deckCards.remove(deckCards.size()-1);			
					}
				}
				else
			...
			for(i=0;i<3;i++) 
			{
				if(user3.shouldDrawCard(mainCard, decCard))
				{
					if(deckCards.size()!=0) 
					{
						user3.receiveCard(deckCards.get(deckCards.size()-1));	
						deckCards.remove(deckCards.size()-1);
			...
			for(i=0; i<3; i++) 
			{
				...
				{
					if(deckCards.size()!=0) {
						user4.receiveCard(deckCards.get(deckCards.size()-1));		
						deckCards.remove(deckCards.size()-1);			
					}
				}
				else
